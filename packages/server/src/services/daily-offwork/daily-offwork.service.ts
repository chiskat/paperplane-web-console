    ...
    if (mode !== 'send') {
      ...
      for (const company of companies) {
        ...
        for (const setting of company.allOffworkNoticeSettings) {
          ...
          if (!workplaceRecord) {
            workplaceRecord = await this.recorder.recordWorkplace(workdayRecord, setting.belongToWorkplace)
          ...
          if (!isRepeat) {
            ...
            const imageUrl = await this.recorder.offworkViewToImage(viewUrl, `/offwork-image/${record.id}.png`)
            ...
            this.logger.log(` 公司 [${company.id}] 工作地 [${workplaceRecord.id}] 本次已获取过，命中缓存`)
          }
        }
      }
